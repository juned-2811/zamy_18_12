...
abstract class REST_Controller extends CI_Controller {
    ...
    protected function _prepare_basic_auth()
    {
        ...
        $http_firetoken = $this->input->server('HTTP_FIRE_TOKEN') ?: $this->input->server('HTTP_FIRE_TOKEN');
        $http_type = $this->input->server('HTTP_TYPE') ?: $this->input->server('HTTP_TYPE');
        $user_id = $this->input->post('user_id') ?: $this->input->post('user_id');
		
		if(!empty($user_id)){
			
			/* if(empty($http_firetoken)){
				 // Display an error response
					$this->response([
						'status' => 0,
						'message' => 'fire_token is required in header.'
					], self::HTTP_UNAUTHORIZED);
			}
			if(empty($http_type)){
				 // Display an error response
					$this->response([
						'status' => 0,
						'message' => 'type is required in header.'
					], self::HTTP_UNAUTHORIZED);
			} */
			if($http_type == 'customer'){
				
				$user_data['conditions'] 	= array('id'=>$user_id); 
				$user_data['returnType'] 	= 'single';
				$user_data['fields'] 		= 'fire_token';
				$result 				 	= $this->common_model->getRows('online_customer',$user_data);
				 
				if($http_firetoken != $result['fire_token']){
					 // Display an error response
					$this->response([
						'status' => 2,
						'message' => 'fire_token invalid'
					], self::HTTP_UNAUTHORIZED);
				}
			}
			elseif($http_type == 'delivery'){
				
				$user_data['conditions'] 	= array('id'=>$user_id); 
				$user_data['returnType'] 	= 'single';
				$user_data['fields'] 		= 'fire_token';
				$result 				 	= $this->common_model->getRows('delivery_boy',$user_data);
