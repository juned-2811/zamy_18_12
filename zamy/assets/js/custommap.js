	...
	google.maps.event.addListener(places, 'place_changed', function () {
		...
		for (var i = 0; i < place.address_components.length; i++) {
			...
			if(place.address_components[i].types[0] == 'sublocality_level_1'){
                document.getElementById('searched_area').innerHTML = place.address_components[i].long_name;
                document.getElementById('current_area').innerHTML = place.address_components[i].long_name;
		...
		$('#search_address').val(place.formatted_address);
		
		document.getElementById('current_address').innerHTML = place.formatted_address
...
function initialize(latitude,longitude) {
  ...
  google.maps.event.addListener(marker, 'dragstart', function() {
	...
	geocodePosition(marker.getPosition(),latitude,longitude);
		...
		for (var i = 0; i < responses[0].address_components.length; i++) {
			...
			console.log(responses[0].address_components[i].long_name);
			 ...
			 if(responses[0].address_components[i].types[0] == 'political'){
                document.getElementById('searched_area').innerHTML = responses[0].address_components[i].long_name;
               // document.getElementById('current_area').innerHTML = responses[0].address_components[i].long_name;
				
				current_area = responses[0].address_components[i].long_name
            }
			
			if(responses[0].address_components[i].types[0] == 'postal_code'){
                document.getElementById('postal_code').innerHTML = responses[0].address_components[i].long_name;
				
				pincode = responses[0].address_components[i].long_name
            }
            if(responses[0].address_components[i].types[0] == 'country'){
                document.getElementById('country').innerHTML = responses[0].address_components[i].long_name;
            }
        }
...
function updateMarkerStatus(str,current_area) {
  ...
  //document.getElementById('current_address').innerHTML = str
  //document.getElementById('current_area').innerHTML = current_area
